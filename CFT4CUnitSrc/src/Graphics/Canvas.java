...
public class Canvas extends JPanel
{
    ...
    public void createGraphFromNodeList(String nodeStrList)
    {
        ...
        for (int i = 0; i < nodeStrs.length; i++)
        {
            ...
            Node node = new Node(Integer.valueOf(nodeData[0]), nodeData[1], Integer.valueOf(trueIdStr), Integer.valueOf(falseIdStr), this);
            nodeList.add(node);
        }
        ...
        for (int i = 0; i < nodeList.size(); i++)
        {
            ...
            if (node.isCondition())
            {
                ...
                {
                    falseNode.setLocation(x + marginX - falseNode.getWidth() / 2, y + marginY - falseNode.getHeight() / 2);
                }
            } else
            {
                if (!(trueNode.isLocationSet() && trueNode.isCondition()))
        ...
        for (int i = 0; i < nodeList.size(); i++)
        {
            ...
            for (int j = 0; j < targets.size(); j++)
            {
                ...
                } else
                {
                    ...
                    boolean outOfPadding = (source.getX() + source.getWidth() < target.getX() || target.getX() + target.getWidth() < source.getX()) && distance > gap + 1;
