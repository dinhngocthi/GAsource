...
class GenerateDeclareFun {
	...
	protected GenerateDeclareFun(ArrayList<Bien> variableList) {
		this.variableList = variableList;
	}

	protected GenerateDeclareFun(String testcases) {
	...
	public static void main(String[] args) {
		GenerateDeclareFun g = new GenerateDeclareFun("double a[][],double b");
		g.run();
		System.out.println(g.getOutput());
	}

	private ArrayList<Bien> convertStringList(String testcases) {
		...
		for (String testcase : testcaseList) {
			...
			if (testcase.contains("]")) {
				...
				if (testcase.contains("][")) // nếu là mảng hai chiều
					switch (type) {
					case "int":
